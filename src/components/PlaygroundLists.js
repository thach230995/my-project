...
function PlaygroundInfo({ id, name, address, price }) {
  ...
  return (
    <div className=" border p-4 my-4 rounded-md shadow-lg shadow-cyan-500/50 mt-4 hover:bg-red-50">
      ...
      <Link to={`/pitch/${id}`} className="button bg-blue-500 text-white px-4 py-2 rounded-md shadow-md mt-4 hover:bg-blue-700 ">Chi tiết sân</Link>
    ...
    <div className='flex flex-row gap-4'>
      ...
      <div className='flex flex-1 flex-col'>
        {locations.map(location => (
          <div key={location.id}>
            <h1 className="text-2xl text-orange-400 font-semibold mb-4">Danh sách sân: {location.name}</h1>
            <div className="grid grid-cols-3 gap-4">
              {location.pitch && location.pitch.map((pitch) => (
